...
const baselinePath = argValue(args, "--baseline") ?? process.env.COPILOT_EVAL_BASELINE ?? join("eval-results", "meal-core-baseline.json");
const workDir = argValue(args, "--work-dir") ?? process.env.COPILOT_EVAL_IMPROVE_LOOP_DIR ?? join("eval-results", "meal-core-improve-loop");
...
for (let iteration = 1; iteration <= maxIterations; iteration += 1) {
  ...
  console.log(`Meal-core improve loop iteration ${iteration}/${maxIterations}`);
  runEval([
    "loop",
...
function printReport(report: ImproveLoopReport, path: string) {
  console.log(`Meal-core improve loop: ${report.stopReason}`);
